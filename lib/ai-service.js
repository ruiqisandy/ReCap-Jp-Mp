...
IMPORTANT: Return ONLY the JSON object, no extra text or markdown.`;
  ...
  /**
   * Generate 5-10 suggested labels from all chat summaries in a single step
   * Uses shorter headline-based summaries to fit more chats in one API call
   *
  ...
  async generateLabelsFromChatSummaries(chats) {
    try {
      ...
      console.log('[AI Service] Analyzing', chatsWithSummaries.length, 'chats with summaries using single-step approach');
  ...
  /**
   * Generate 5-10 labels for all chats in a single step
   ...
   * @returns {Promise<Array<Object>>} Array of 5-10 labels
   * @private
   */
  async _generateLabelsForAllChats(chats) {
    try {
      // Headlines are shorter (12-22 words), so we can use tighter limits
      const MAX_HEADLINE_LENGTH = 150; // Headlines should be ~100 chars max
      const MAX_TITLE_LENGTH = 80; // Tighter title limit

      // Prepare data for analysis - use headlines only
      // Headlines are much shorter than summaries, allowing us to process more chats
      ...
      // Log headline and title lengths for debugging
      ...
      const avgHeadlineLength = Math.round(totalHeadlineLength / chatData.length);
      const avgTitleLength = Math.round(totalTitleLength / chatData.length);

      console.log(`[AI Service] Processing ${chatData.length} chats: headlines=${totalHeadlineLength} chars (avg ${avgHeadlineLength}), titles=${totalTitleLength} chars (avg ${avgTitleLength})`);

      // Warn if any headlines or titles were truncated
      ...
      if (truncatedHeadlines.length > 0) {
        console.log(`[AI Service] Truncated ${truncatedHeadlines.length} long headlines (>${MAX_HEADLINE_LENGTH} chars)`);
      }
      ...
      const prompt = `Analyze ${chatData.length} AI chat headlines and extract 5-10 thematic topic labels:

${chatData.map((chat, i) => `${i + 1}. [${chat.platform}] ${chat.headline}`).join('\n')}

Group related chats by identifying common themes. Return a JSON array with:
- name: Label name (2-5 words)
- description: One sentence description (50-80 chars)
- conversationIds: Array of chat numbers (e.g., [1, 3, 7])
- confidence: 0-1 score

Format:
[{"name":"Topic Name","description":"Brief description","conversationIds":[1,2,3],"confidence":0.85}]
...
Return ONLY the JSON array, no extra text.`;
      ...
      if (prompt.length > 10000) {
        ...
        console.warn(`[AI Service] Headlines total: ${chatData.reduce((sum, c) => sum + c.headline.length, 0)} chars (avg ${avgHeadlineLength})`);
      }
