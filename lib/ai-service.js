...
IMPORTANT: Return ONLY the JSON array, nothing else.`;
  ...
  async _consolidateLabels(rawLabels, allChats) {
    try {
      const MAX_LABEL_DESC_LENGTH = 200; // Truncate long descriptions

      // Truncate label descriptions to prevent quota errors
      const truncatedLabels = rawLabels.map(label => {
      ...
      console.log(`[AI Service] Consolidation input: ${truncatedLabels.length} labels, total=${totalLength} chars`);
