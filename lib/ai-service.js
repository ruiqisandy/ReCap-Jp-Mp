...
Return ONLY the JSON array, no extra text.`;
  ...
  async _classifyChatsIntoPreferredLabels(chats, preferredLabels) {
    try {
      ...
      const prompt = `You are organizing AI chat summaries using only the user's preferred labels.
...
Instructions:
- Use exactly the preferred labels listed above; do not invent new labels.
- Only assign a chat to a label if it clearly fits the topic.
- If a label has no matching chats, return it with an empty "conversationIds" array.
- Return a JSON array where each object includes:
  - "name": the label name exactly as provided
  - "description": a short summary (<=80 characters) of the grouped chats
  - "conversationIds": an array of chat numbers (e.g., [1, 4])
  - "confidence": a number between 0 and 1 representing confidence in the grouping
