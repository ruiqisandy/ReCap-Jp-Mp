...
Return ONLY the summary text, no extra formatting or labels.`;
  ...
  /**
   * Extract conceptual themes from conversations (Phase 1)
   * Analyzes all chats to identify core concepts and themes related to the label
   *
   * @param {Array<Object>} chatsWithContext - Array of chat objects with summaries
   * @param {string} labelName - Name of the label (for context)
   * @returns {Promise<Array<Object>>} Array of theme objects with {name, description, keyQuestions}
   * @private
   */
  async _extractConceptualThemes(chatsWithContext, labelName = '') {
    try {
      console.log(`[AI Service] Extracting conceptual themes for label: ${labelName}`);

      // Prepare enriched context with message pair samples
      const MAX_PAIRS_PER_CHAT = 3; // Sample top message pairs for deeper context
      const MAX_SUMMARY_LENGTH = 250;

      const enrichedChats = chatsWithContext.map((chat, index) => {
        const truncatedSummary = chat.chatSummary.length > MAX_SUMMARY_LENGTH
          ? chat.chatSummary.substring(0, MAX_SUMMARY_LENGTH) + '...'
          : chat.chatSummary;

        // Sample key message pairs for richer context
        let pairSamples = '';
        if (chat.messagePairSummaries && chat.messagePairSummaries.length > 0) {
          const sampled = chat.messagePairSummaries.slice(0, MAX_PAIRS_PER_CHAT);
          pairSamples = '\n   Key exchanges: ' + sampled.map((p, i) => `${i + 1}) ${p}`).join('; ');
        }

        return `${index + 1}. [${chat.platform}] "${chat.title}"
   Overall: ${truncatedSummary}${pairSamples}`;
      }).join('\n\n');

      const prompt = `You are analyzing ${chatsWithContext.length} conversations grouped under the label "${labelName}".

Your goal is to extract the core conceptual themes and areas of inquiry that define this knowledge domain, NOT just to categorize individual chats.

Here are the conversations with their summaries and key exchanges:

${enrichedChats}

Task:
Identify 3-7 fundamental conceptual themes that characterize this label's knowledge space. Think about:
- What overarching questions or problems do these conversations address?
- What concepts, frameworks, or domains appear repeatedly?
- What intellectual territory does this label cover?

For each theme, provide:
- A clear, specific name (2-5 words)
- A 1-2 sentence description of what this theme encompasses
- 2-4 key questions or subtopics within this theme
  ...
  "themes": [
    {
      ...
      "keyQuestions": [
        "Specific question or subtopic 1",
        "Specific question or subtopic 2",
        "Specific question or subtopic 3"
      ]
    }
  ]
}

Focus on intellectual coherence and conceptual clarity. Return valid JSON only, no markdown or commentary.`;
      ...
      try {
        ...
        themesData = JSON.parse(cleanedResponse);

        if (!themesData.themes || !Array.isArray(themesData.themes)) {
          throw new Error('Invalid response structure: missing themes array');
        }

        console.log(`[AI Service] Extracted ${themesData.themes.length} conceptual themes`);
        return themesData.themes;

      } catch (parseError) {
        console.error('[AI Service] Error parsing themes JSON:', parseError);
        console.error('[AI Service] Response was:', response);
        throw new Error('Failed to parse theme extraction response: ' + parseError.message);
      }

    } catch (error) {
      console.error('[AI Service] Error extracting conceptual themes:', error);
      // Return a simple fallback theme
      return [{
        name: labelName || 'General Discussion',
        description: 'Various topics and discussions',
        keyQuestions: ['What are the main topics?', 'What patterns emerge?']
      }];
    }
  },
  ...
  async generateBulletPoints(chatsWithContext, labelName = '') {
    try {
      ...
      // PHASE 1: Extract conceptual themes (new approach)
      console.log('[AI Service] Phase 1: Extracting conceptual themes...');
      const themes = await this._extractConceptualThemes(chatsWithContext, labelName);

      // PHASE 2: Build mindmap around these themes
      console.log('[AI Service] Phase 2: Building mindmap structure...');

      // Prepare compact chat summaries for mapping to themes
      ...
      const chatList = chatsWithContext.map((chat, index) => {
        ...
        return `${index + 1}. [${chat.platform}] "${chat.title}" - ${truncatedSummary}`;
      }).join('\n');

      // Format themes for the prompt
      const themesList = themes.map((theme, index) => {
        const questions = theme.keyQuestions.map(q => `    • ${q}`).join('\n');
...
${questions}`;
      ...
      const prompt = `You are creating a concept-centered mind map for the label "${labelName}".

PHASE 1 ANALYSIS identified these ${themes.length} core conceptual themes:

${themesList}

Now map the ${chatCount} conversations below as examples/evidence for these themes:

${chatList}

Goal:
Build a hierarchical mindmap CENTERED on the conceptual themes above, with individual chats serving as supporting examples.

Guidelines:
1. Use the ${themes.length} themes as your primary organizing structure (top-level topics)
2. For each theme, develop subtopics based on the keyQuestions/aspects identified
3. Map relevant chats as concrete examples under appropriate (sub)topics
4. For each chat example, provide ONLY:
   - chatIndex: the chat number
   - detail: the specific question, insight, or topic from that conversation (keep it concise)
5. Do NOT add extra synthesis or summaries for individual chats - just link them with their specific detail
6. Show conceptual relationships with "relatedTo" when themes/subtopics connect
7. Allow natural depth variation: detailed where content is rich, simpler where focused
8. Prioritize conceptual coherence over chat coverage
...

{
  "mindmap": [
    {
      "topic": "Theme Name (from Phase 1)",
      "summary": "1-2 sentence synthesis of this conceptual area",
      "subtopics": [
        {
          "name": "Subtopic/Question from keyQuestions",
          "summary": "Brief elaboration (optional)",
          "examples": [
            {
              "chatIndex": 3,
              "detail": "Specific question or insight from that chat"
            }
          ],
          "relatedTo": ["Other subtopic names if conceptually linked"],
          "subtopics": [/* Allow nesting for complex areas */]
        }
      ]
    }
  ]
...
Requirements:
- Organize around CONCEPTS first, chats second
- Support 1-4 levels of depth based on content richness
- Not all chats need to appear (quality > coverage)
- A chat may appear multiple times if it addresses multiple themes
- Keep chat examples simple: just chatIndex + detail (no extra "title" field)
